    ...
    with tqdm(total=per_len, desc='Iteration %d' % i) as pbar:
        for i_episode in range(per_len):
            ...
            while not done:
                ...
                # 选择占有率最低的
                # 不选0 [0, -0.1, -0.3, 0]
                cpu_chose = env.pcpu * mask_mac1
                cpu_chose[cpu_chose == 0] = np.amax(cpu_chose) + 1
                index = np.argmin(cpu_chose)

                # print(cpu_chose)
                action1 = np.where(cpu_chose == cpu_chose.max())[0][0]
                # print(action1)
            ...
            #    stop_list[cnt]), stop_r, reward_kind, step_r, env.stop)
