    ...
    with tqdm(total=per_len, desc='Iteration %d' % i) as pbar:
        for i_episode in range(per_len):
            ...
            while not done:
                ...
                # sort_cpu = copy.deepcopy(list(s_cpu))
                # sort_cpu.sort()

                # 每次选择不为0的CPU最大的服务
                # [1, 0, 0, 1, 0]
                is_list = (env.mac_sev[action1] >= np.ones(env.mac_sev.shape[1])).astype(int)
                # [cpu1, 0, 0, cpu4, 0]
                cpu_list = s_cpu * is_list
                #max_index
                action2 = np.where(cpu_list == cpu_list.max())[0][0]
                ...
                action3 = agent3.take_action(
                    state, mask_mac2, True, env)
            ...
            if sum(env.stop) > b_stop or (sum(env.stop) == b_stop and sum(env.now_trf) > b_stop_trf):
                torch.save(agent1.actor.state_dict(), './model/a3_1a_bt_under_bs.pt')
                torch.save(agent1.critic.state_dict(), './model/a3_1c_bt_under_bs.pt')
                torch.save(agent2.actor.state_dict(), './model/a3_2a_bt_under_bs.pt')
                torch.save(agent2.critic.state_dict(), './model/a3_2c_bt_under_bs.pt')
                torch.save(agent3.actor.state_dict(), './model/a3_3a_bt_under_bs.pt')
                torch.save(agent3.critic.state_dict(), './model/a3_3c_bt_under_bs.pt')

                b_stop = sum(env.stop)
                b_stop_trf = sum(env.now_trf)

                for tmp in range(0, len(transition_dict['mac_sev']), 50):
                    np.savetxt('./BS_state_change_a3/BS_' + str(tmp) +  '.csv', transition_dict['mac_sev'][tmp], delimiter=',')
                np.savetxt('./BS_state_change_a3/BS_last' +  '.csv', transition_dict['mac_sev'][-1], delimiter=',')

            if sum(env.now_trf) > b_trf or (sum(env.stop) > b_stop and sum(env.now_trf) == b_stop_trf):
                torch.save(agent1.actor.state_dict(), './model/a3_1a_bs_under_bt.pt')
                torch.save(agent1.critic.state_dict(), './model/a3_1c_bs_under_bt.pt')
                torch.save(agent2.actor.state_dict(), './model/a3_2a_bs_under_bt.pt')
                torch.save(agent2.critic.state_dict(), './model/a3_2c_bs_under_bt.pt')
                torch.save(agent3.actor.state_dict(), './model/a3_3a_bs_under_bt.pt')
                torch.save(agent3.critic.state_dict(), './model/a3_3c_bs_under_bt.pt')
            ...
            np.savetxt('./last_state_a3/' + str(cnt) + 'last' +  '.csv', env.mac_sev, delimiter=',')
...
rl_utils.show(env, stop_list, trf_list, dids_list, mac_kind, sev_kind,
              mac_sev_list, cpus, mems, return_list, rank, reward_kind, 32, para[13], 'para3', punish_list)
